...
現在は packaged build をテスターへ渡し、実データの反馈待ちに入る段階。
...
- 数値 UI は `mm` 表示ベース
- AO remap は output セクションで調整し、保存 PNG に反映する
...
- packaged build は生成済み
- テスター向け手順書は整備済み
- 次の主作業はテスター反馈回収
- 実装の次候補は `AO Profile` の JSON 化設計
...
- AO output は canvas ベース描画で初回表示のぼやけを回避
...
- GitHub 公開向けに `.gitignore` / `.gitattributes` / `.env` の最低限を整理
...
- 自動テストは未整備
- 実メモリリーク有無は stress test ベースの最終確認がまだ必要
...
5. `AO Profile` JSON 化の schema を設計する
6. 最低限の bake regression test を入れる
